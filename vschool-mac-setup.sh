...
do
    echo -e "${lightblue}Whoops, looks like you entered something wrong. Please try again (first and last names): ${clear} "
    read first_name last_name
done
...
if [ $(echo $0) != "/bin/bash" ]
then
    echo -e "${lightblue}For simplicity, I'm switching your default shell to bash. If you want to keep another shell, please speak with an instructor for help.${clear}"
    chsh -s /bin/bash
#else
#    echo -e "${lightblue}I just checked if you're using bash for your shell program, and it looks like you are! ${thumbs_up}${clear}"
fi

# Todo: Turn this into a while loop so they can't move ahead until it's installed
if [ ! -d /Applications/Google Chrome.app ]
    then
        ...
        read -p "${lightblue}Once you've finished installing Chrome, hit return to continue (return)${clear} "
    fi

read -p "${lightblue}Now I'll be creating an SSH key for you. (return)${clear}"
read -p "${lightblue}This is useful for communicating with external services, like GitHub (return)${clear}"

# Check for an existing SSH Key and create a new one if it doesn't exist
if [ -f ~/.ssh/id_rsa.pub ]
then
    echo
    read -p "${lightblue}Actually, it looks like you've already got an SSH file, which is great! ${thumbs_up} (return)${clear}"
...
read -p "${lightblue}Then simply paste (${mac_command_symbol} + P) the SSH key (which is already copied for you) into the 'Key' input box and hit the green 'Add SSH key' button (return)${clear}"
...
read -p "${lightblue}(If you happened to get an error that said the SSH key is already in use, you're all set!)${clear}"


# Install Homebrew
read -p "${lightblue}How did that go? It wasn't too bad, was it? (return)${clear}"
...
read -p "${lightblue}It makes installing (and switching versions of) Node.js very easy (return)${clear}"
...
read -p "${lightblue}${sweat_smile}${sweat_drops} You're enjoying watching me do all the work, aren't you? (return)${clear}"
...
read -p "${lightblue}${sweat_smile}${sweat_drops} Alright! That's everything! (return)${clear}"
read -p "${lightblue}It's been nice working with you, ${first_name}! (return)${clear}"
read -p "${lightblue}Hit return to close the Terminal app. Once you reopen it, everything should be all set up for you. Good luck with your course!${clear}"
